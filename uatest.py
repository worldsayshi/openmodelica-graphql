...
_logger = logging.getLogger('My asyncua')
...
async def main():
    ...
    async with Client(url=url) as client:
        ...
        _logger.info('Namespaces are %r', await client.get_namespace_array());

        uri = 'http://opcfoundation.org/UA/'
        idx = await client.get_namespace_index(uri)
        _logger.info('Got idx %r', idx);
        _logger.info('Children info: %r', await client.nodes.root.get_children_descriptions())
